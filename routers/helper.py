...
- 3개는 서로 완전히 다른 방향이어야 함:
  1번: 감정/경험의 깊이 탐색 (내담자 감정의 결을 파고들기)
...
[출력 형식 - JSON만 출력, 마크다운/설명/코드블록 절대 금지]
{
...
def _normalize_reply_candidates(cands: Any) -> Optional[List[Dict[str, str]]]:
    """
    HCX가 문자열 배열로 reply_candidates를 반환하는 경우를
    {"text": ..., "why": ...} 형식으로 정규화한다.
    정규화 불가능하면 None 반환.
    """
    ...
    for i, c in enumerate(cands):
        if isinstance(c, dict):
            normalized.append(c)
        elif isinstance(c, str) and len(c.strip()) >= 8:
            normalized.append({"text": c.strip(), "why": f"HCX 제안 {i + 1}"})
        else:
            return None
    return normalized
...
def _is_valid(obj: Any) -> bool:
    ...
    if not _valid_reply_candidates(obj.get("reply_candidates")):
        # 문자열 배열로 온 경우 정규화 후 재검증
        normalized = _normalize_reply_candidates(obj.get("reply_candidates"))
        if normalized is None:
            return False
        obj["reply_candidates"] = normalized
        if not _valid_reply_candidates(obj["reply_candidates"]):
            return False
...
def analyze_with_hcx(
    ...
    user_content = (
        f"[이전 대화]\n{history_block}\n\n"
        f"[직전 상담사 발화]\n{counselor_block}\n\n"
        f"[현재 내담자 발화 - 이 발화에 집중하여 reply_candidates 3개 작성]\n{client_text.strip()}"
    )
